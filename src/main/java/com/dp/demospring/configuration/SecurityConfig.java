...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception
  {
      ...
      http.csrf(AbstractHttpConfigurer::disable).
              authorizeHttpRequests(auth -> auth.requestMatchers("/JournalEntry/**","/UserEntry/**")
                      .authenticated()
                      .requestMatchers("/admin/**").hasRole("ADMIN")
                      .anyRequest().permitAll()).formLogin(AbstractAuthenticationFilterConfigurer::permitAll // Enable default login page
                      )
